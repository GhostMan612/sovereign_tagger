...
| Audio | `AudioService` queue mutators must `await` the matching `player.*AudioSource*` call inside `_withQueueLock`; never mutate `playlist.value` alone. UI sounds go only through `Sfx` / `TapFeedback` (native `SoundPool`, `USAGE_GAME`, no audio focus) — never play UI sounds through `just_audio` (it grabs the audio session and ducks music). |
| Tap FX | `TapFxLayer` (MaterialApp `builder`) gives every tappable pulse/ring/burst + tap sound automatically — do not hand-roll per-button bursts. Tappable = semantics `onTap` (InkWell/Material buttons) or `GestureDetector` `onTap`; tint a control with `TapFxTint` or `CyberTapFeedback(fxColor:)`. Theme splash is `CyberInk.splashFactory`. |
| Shaders | Fragment shaders live in `shaders/*.frag`, are listed under `flutter: shaders:` in `pubspec.yaml`, and are loaded once via `FragmentProgram.fromAsset` with a non-shader fallback. `setFloat` indices follow uniform declaration order (a `vec2` takes 2 slots, `vec4` 4). Compile-check with `impellerc --runtime-stage-gles` / `--runtime-stage-vulkan` / `--sksl` before shipping. |
| Tabs & routes | Tabs switch through `AnimatedTabStack` (never swap its wrapper types — that resets tab state); page transitions come from `CyberPageTransitionsBuilder` in the theme, so plain `MaterialPageRoute`s animate automatically. |
