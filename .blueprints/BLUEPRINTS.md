...
- G31: Ghost NLU keeps two views of the input — canonical words (synonyms + typo correction) for intent, raw words for phrases and library search. Mapping first broke things: 'stop' → 'pause' hid 'stop after this song', 'love' → 'favorite' made 'love you' favorite the song and would rewrite an artist named Love. BM25 title bonuses must skip titles that are all stop words, and help entries whose text lists other topics as examples must be indexed by title/keys only.
- G32: A tab switcher that keeps tabs alive must give every slot the same wrapper chain every build (`Offstage` → `TickerMode` → `IgnorePointer` → `Opacity` → `Transform`) and only change the values; swapping wrapper types between active/leaving/hidden rebuilds the subtree and resets tab state. `IndexedStack` alone keeps hidden tabs' tickers running — wrap slots in `TickerMode(enabled: active)`.
- G33: Fragment shaders: declare under `flutter: shaders:`, `#include <flutter/runtime_effect.glsl>`, use `FlutterFragCoord()`, and feed uniforms with `setFloat` in declaration order. `flutter_tester` can load and render them, and the SDK's `impellerc` compile-checks the GLES/Vulkan/SkSL targets offline. Thresholded value noise makes smeared blobs; use per-cell hashed dots for sparks, and scale grid line widths by the perspective derivative.
